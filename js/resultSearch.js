...
if(keySearch != '') {
    ...
    $('.content-wrapper').each(function(ind, el) {
        if($(el).children('h5').text().toLowerCase().includes(keySearch.toLowerCase())) {
...
reLoad = () => {
    ...
    $('.heart-wishlist').click(function(){
        if($(this).hasClass('fal')){
            $('.ds-wishlist-with-count').text(Number.parseInt($('.ds-wishlist-with-count').text()) + 1);
...
filter = (start, end) => {
    ...
    $('.content-wrapper').each(function() {
        var price = Number($(this).find('.price').text().replace(/[VND.]/g, ''));
...
sortNameAZ = () => {
    ...
    for(i = 0; i < product_After_Filter_3.length - 1; i++){
        for(j = product_After_Filter_3.length - 1; j > i ; j--){
            var textI = product_After_Filter_3[j].substring(product_After_Filter_3[j].indexOf('<h5>') + 4, product_After_Filter_3[j].lastIndexOf('</h5>'));
            var textJ = product_After_Filter_3[j-1].substring(product_After_Filter_3[j-1].indexOf('<h5>') + 4, product_After_Filter_3[j-1].lastIndexOf('</h5>'));
...
sortPriceLtoH = () => {
    ...
    for(i = 0; i < product_After_Filter_3.length - 1; i++){
        for(j = product_After_Filter_3.length - 1; j > i ; j--){
            var textI = product_After_Filter_3[j].substring(product_After_Filter_3[j].indexOf('<p class="price">') + 17, product_After_Filter_3[j].lastIndexOf('VND</p>'));
            var textJ = product_After_Filter_3[j-1].substring(product_After_Filter_3[j-1].indexOf('<p class="price">') + 17, product_After_Filter_3[j-1].lastIndexOf('VND</p>'));
    ...
    $('.btn-addCart').click(function() {
        ...
        if(arrProduct != undefined) {
            ...
            arrProduct.forEach((el) => {
                if(el.link_img == $(thisBTN).parents('.content-wrapper').find('img').attr('src')
                && el.name_product == $(thisBTN).parents('.content-wrapper').find('h5').text()
                && el.price_product == $(thisBTN).parents('.content-wrapper').find('.price').text()) {
                    check = false;
                    return;
                }
            })
            ...
            if(check){
                ...
                var product = {
                    'link_img': $(thisBTN).parents('.content-wrapper').find('img').attr('src'),
                    'name_product': $(thisBTN).parents('.content-wrapper').find('h5').text(),
                    'price_product': $(thisBTN).parents('.content-wrapper').find('.price').text()
                }
