$(document).ready(function () {
  ...
  reload = () => {
    ...
    if ($(".real-price").length > 0) {
      ...
      $(".cart-product-wrapper").html(
        '<img src="../images/no-data-animate.svg" alt="">'
      );
    }
  ...
  $(".chose-amount").each(function (ind, el) {
    $(el).change(function () {
      var root_Price = Number.parseFloat(
        $(el)
          .siblings(".root-price")
          .text()
          .replace(/[VND.]/g, "")
      ...
      $(el)
        .siblings(".real-price")
        .text(
          (root_Price * $(el).children("option:selected").val()).toLocaleString(
            "de-DE"
          ) + " VND"
        );
  ...
  reloadAfterAddCode = () => {
    arrCodeSale.forEach((el) => {
      if (el == $(".inp-code-sale").val()) {
        var tmp_total = Number.parseFloat(
          document
            .getElementsByClassName("total")[0]
            .textContent.replace(/[VND.]/g, "")
        );
  ...
  $(".del-cart-item").click(function () {
    $(this).parents(".cart-item-wrapper").remove();
